...
class TaDistanceSolver(KRCBSSolver):
    ...
    def __init__(self, my_map, starts, goals, k=0):
        ...
        goal_heuristics = [compute_heuristics(my_map, goal) for goal in goals]
        agent_goal_costs = {agent_id: [] for agent_id in range(self.num_of_agents)}

        for agent_id, start in enumerate(starts):
            for goal_id in range(len(goals)):
                agent_goal_costs[agent_id].append(goal_heuristics[goal_id].get(start, float('inf')))

        assignment = hungarian_algorithm(agent_goal_costs)
        self.goals = [goals[assignment[agent_id]] for agent_id in range(self.num_of_agents)]
